...
public class ExpressionAddOperators {
    ...
    public void helper(List<String> rst, String path, String num, int target, int pos, long currVal, long prevVal) {
        if (pos == num.length()) {
            if (target == currVal)
                rst.add(path);
            return;
        ...
        for (int i = pos; i < num.length(); i++) {
            ...
            if (pos == 0) {
                helper(rst, path + n, num, target, i + 1, n, n);
            } else {
                helper(rst, path + "+" + n, num, target, i + 1, currVal + n, n);

                helper(rst, path + "-" + n, num, target, i + 1, currVal - n, -n);

                helper(rst, path + "*" + n, num, target, i + 1, currVal - prevVal + prevVal * n, prevVal * n);
            }
        }
    }
